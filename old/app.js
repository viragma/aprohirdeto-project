...
function renderAds(ads) {
    ...
    try {
        ...
        console.log('Gallery innerHTML beállítva, hossza:', gallery.innerHTML.length);
        
        // Ellenőrzés
        const renderedCards = gallery.querySelectorAll('.ad-card');
        console.log(`Renderelt kártyák száma: ${renderedCards.length}`);
        
        // DEBUG: Nézd meg 2 másodperc múlva is
        setTimeout(() => {
            console.log('🔍 DEBUG 2 másodperc múlva:');
            console.log('Gallery innerHTML hossza:', gallery.innerHTML.length);
            console.log('Gallery innerHTML első 200 karakter:', gallery.innerHTML.substring(0, 200));
            const cards = gallery.querySelectorAll('.ad-card');
            console.log('Kártyák száma:', cards.length);
            if (cards.length === 0) {
                console.error('⚠️ A KÁRTYÁK ELTŰNTEK!!!');
